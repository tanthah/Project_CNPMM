          ...
          <div>
            {productComments.map((c) => (
              <div key={c._id} className="border-bottom pb-3 mb-3">
                ...
                {/* Admin Reply */}
                {c.adminReply && (
                  <div className="mt-2 ms-5 p-3 rounded" style={{ backgroundColor: '#f8f9fa', borderLeft: '4px solid #0d6efd' }}>
                    <div className="d-flex align-items-center mb-1">
                      <strong className="text-primary me-2">
                        <i className="bi bi-person-badge-fill me-1"></i>
                        Quản trị viên
                      </strong>
                      <small className="text-muted">{new Date(c.adminReply.repliedAt || Date.now()).toLocaleString('vi-VN')}</small>
                    </div>
                    <p className="mb-0 text-dark">{c.adminReply.content}</p>
                  </div>
                )}
              </div>
            ))}
