    ...
    useEffect(() => {
        ...
        socket.current.on('chat_history', (history) => {
            const historyMsgs = history.map(h => ({
                type: h.from === 'user' ? 'user' : 'bot',
                content: h.message,
                timestamp: new Date(h.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })
            }));
            // Prepend or set messages? Set is safer for history load
            // But we need to keep the initial welcome message?
            // Let's keep initial welcome msg if it exists in state 
            setMessages(prev => {
                const initial = prev[0]; // Assuming first is welcome
                return [initial, ...historyMsgs];
            });
        });

        socket.current.on('receive_message', (data) => {
            // Ignore own messages to prevent duplication (since we add them optimistically)
